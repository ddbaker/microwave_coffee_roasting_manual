...
assert.equal(htmlFiles.length, 15, 'Expected 12 chapters, 2 home pages, and 404');
const expectedPages = new Set(['index.html', 'ja/index.html', '404.html', ...['en', 'ja'].flatMap(lang => chapters.map(ch => `${lang}/${ch.key}/index.html`))]);
for (const file of files) {
  const relative = path.relative(root, file).replaceAll('\\', '/');
  assert(expectedPages.has(relative) || relative === 'LICENSE.txt' || /^_astro\/[\w.-]+\.css$/.test(relative) || imageNames.some(name => relative === `images/${name}`), `Unexpected published file: ${relative}`);
  assert((await stat(file)).size < 25 * 1024 * 1024, `Asset exceeds Pages limit: ${relative}`);
}
for (const file of htmlFiles) {
  const html = await readFile(file, 'utf8');
  assert(!/<script\b/i.test(html), `Client JavaScript in ${file}`);
...
console.log(`Verified: ${htmlFiles.length} HTML pages, ${imageNames.length} referenced images, all local links and anchors, matching language links, no client JavaScript, publication allowlist and file limits.`);
